...
public class HologramMythicMobMechanic extends MechanicComponent {
    ...
    public HologramMythicMobMechanic(ConfigurationSection configurationSection) {
    ...
    public boolean execute(LivingEntity caster, int skillLevel, List<LivingEntity> targets, int castCounter, int skillIndex) {
        ...
        for (LivingEntity target : targets) {
            ...
            Entity entity = null;
            try {
                entity = apiHelper.spawnMythicMob(key, baseLocation, 1);
            } catch (InvalidMobTypeException e) {
                GuardiansOfAdelia.getInstance().getLogger().info("getPet mythicmob code error: " + key);
                e.printStackTrace();
            }
            if (entity == null) return false;
            if (!(entity instanceof LivingEntity)) {
                GuardiansOfAdelia.getInstance().getLogger().info("Pet is not LivingEntity, petCode: " + key);
                return false;
            }

            LivingEntity model = (LivingEntity) entity;
